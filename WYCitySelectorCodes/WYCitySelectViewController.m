...
#import "WYSearchBarView.h"
#import "WYSearchResultView.h"
#import "WYPredicateTools.h"
#import "WYCityTableViewCell.h"
#import "WYSystemLocationTools.h"
#import "WYRegexTools.h"
#import "WYChineseToPinyinTools.h"
...
@interface WYCitySelectViewController ()<UITableViewDataSource, UITableViewDelegate, WYSystemLocationToolsDelegate>

@property (strong, nonatomic) UITableView *tableView;

@property (strong, nonatomic) WYSearchBarView *tableHeaderView;
@property (strong, nonatomic) WYSearchResultView *searchResultView;
...
@property (strong, nonatomic) WYSystemLocationTools *wySystemLocationTools;

@end

@implementation WYCitySelectViewController
...
- (void)initialize {
    ...
    // 定位
    self.wySystemLocationTools = [WYSystemLocationTools sharedWYSystemLocationTools];
    self.wySystemLocationTools.delegate = self;
}
...
- (void)tableViewConfiguration {
    ...
    [self.tableView registerClass:[WYCityTableViewCell class] forCellReuseIdentifier:WYCityCollectionViewCell];
...
static NSString * const WYCityCollectionViewCell = @"WYCityCollectionViewCell";
static NSString * const cellReuseID = @"cellReuseID";
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    
    if (indexPath.section <= 3) {
        
        WYCityTableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:WYCityCollectionViewCell forIndexPath:indexPath];
...
#pragma mark - WYSystemLocationToolsDelegate
...
- (WYSearchBarView *)tableHeaderView {
    
    if (!_tableHeaderView) {
        
        _tableHeaderView = [[WYSearchBarView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 44)];
        ...
        _tableHeaderView.textDidChange = ^(NSString *string) {
            ...
            if (string.length != 0) {
                ...
                if ([WYRegexTools validateChinese:string]) {
                    
                    weakSelf.searchResultView.searchResultArray = [NSMutableArray arrayWithArray:[WYPredicateTools searchResultWithSourceData:weakSelf.allCityArray Keywords:string]];
                }else {
                    ...
                    for (NSString *chinese in weakSelf.allCityArray) {
                        ...
                        NSLog(@"%@", [[WYChineseToPinyinTools chineseToPinyinWithChinese:chinese] lowercaseString]);
                        
                        if ([[[WYChineseToPinyinTools chineseToPinyinWithChinese:chinese] lowercaseString] hasPrefix:string]) {
...
- (WYSearchResultView *)searchResultView {
    
    if (!_searchResultView) {
        
        _searchResultView = [[WYSearchResultView alloc] initWithFrame:CGRectMake(0, 64 + 44, kScreenWidth, kScreenHeight - 64 - 44)];
